...
template <typename Key, typename T> void Map<Key, T>::RemoveMap_(Nodes *node) {
  if (node == nullptr) {
    return;
  }
...
typename Map<Key, T>::Nodes *Map<Key, T>::CreateNode_(value_type value,
                                                      Nodes *parent) {
...
template <typename Key, typename T> void Map<Key, T>::FixUp_(Nodes *node) {
  if (node != nullptr) {
...
template <typename Key, typename T>
void Map<Key, T>::Rotate_(Nodes *node, const bool left) {
...
template <typename Key, typename T>
typename Map<Key, T>::Nodes *Map<Key, T>::RemoveNodeCildles2_(Nodes *node) {
...
template <typename Key, typename T>
typename Map<Key, T>::Nodes *Map<Key, T>::RemoveNodeCildles1_(Nodes *node) {
...
template <typename Key, typename T> void Map<Key, T>::RemoveNode_(Nodes *node) {
...
template <typename Key, typename T> bool Map<Key, T>::FixDown_(Nodes *node) {
  bool result = false;
  Nodes *parent = node->parent;
...
template <typename Key, typename T>
std::pair<typename Map<Key, T>::Nodes *, bool>
Map<Key, T>::FindWrite_(const key_type &key, bool passed) {
...
template <typename Key, typename T>
std::pair<typename Map<Key, T>::Nodes *, bool>
Map<Key, T>::FindRead_(const key_type &key, Nodes *search) const {
  if (root_ == nullptr) {
...
template <typename Key, typename T> void Map<Key, T>::Print() const {
  iterator it = begin();
...
template <typename Key, typename T>
bool Map<Key, T>::IsNull_(Nodes *node, int flag) const {
  bool res;
  if (!flag) {
    res = node == nullptr;
  } else if (flag == 1) {
