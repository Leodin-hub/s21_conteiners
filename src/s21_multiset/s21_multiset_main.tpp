...
template <typename Key> MultiSet<Key>::MultiSet(MultiSet &&other) {
  root_ = other.root_;
  begin_ = other.begin_;
  end_ = other.end_;
  size_ = other.size_;
...
template <typename Key>
typename MultiSet<Key>::MultiSet &
...
template <typename Key> bool MultiSet<Key>::operator==(MultiSet &other) const {
...
template <typename Key> bool MultiSet<Key>::empty() const {
...
template <typename Key>
typename MultiSet<Key>::iterator
MultiSet<Key>::find(const value_type &value) const {
...
template <typename Key>
typename MultiSet<Key>::iterator
MultiSet<Key>::lower_bound(const value_type &value) const {
  iterator it = begin();
  for (; it != end(); ++it) {
    if (*it == value) {
      break;
...
template <typename Key>
typename MultiSet<Key>::iterator
MultiSet<Key>::upper_bound(const value_type &value) const {
  iterator it = end();
  for (; it != begin(); --it) {
    if (*it == value) {
      ++it;
...
template <typename Key>
template <class... Args>
std::pair<typename MultiSet<Key>::iterator, bool>
