...
template <typename Key>
typename MultiSet<Key>::Nodes *
MultiSet<Key>::FindWrite_(const value_type &value) {
...
template <typename Key>
std::pair<typename MultiSet<Key>::Nodes *, bool>
MultiSet<Key>::FindRead_(const value_type &value, Nodes *search) const {
  if (root_ == nullptr) {
  ...
  while (search_flag) {
    if (value < search->data) {
      ...
      if (!IsNull_(search->left) && search->left->data == value) {
        search = search->left;
      } else {
        search_flag = false;
      }
...
template <typename Key> void MultiSet<Key>::FixUp_(Nodes *node) {
  if (node != nullptr) {
...
template <typename Key> void MultiSet<Key>::RemoveNode_(Nodes *node) {
...
template <typename Key> bool MultiSet<Key>::FixDown_(Nodes *node) {
  std::cout << "agaagaaga" << std::endl;
  bool result = false;
  Nodes *parent = node->parent;
...
template <typename Key> void MultiSet<Key>::Print() const {
  iterator it = begin();
...
template <typename Key> void MultiSet<Key>::RemoveSet_(Nodes *node) {
  if (node == nullptr) {
    return;
  }
