...
public:
  ...
  class MultiSetIterator : public Iterator {
  public:
    MultiSetIterator(Nodes *node) : Iterator(node) { ; }
    reference operator*() { return this->iterator_->data; }
  };

  class ConstMultiSetIterator : public Iterator {
  public:
    ConstMultiSetIterator(Nodes *node) : Iterator(node) { ; }
...
private:
  ...
  std::pair<Nodes *, bool> FindRead_(const value_type &value,
                                     Nodes *search) const;
...
template <typename Key> class MultiSet<Key>::Node {
public:
...
template <typename Key> class MultiSet<Key>::Iterator {
protected:
  Nodes *iterator_;
