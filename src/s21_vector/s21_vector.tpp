...
template <typename T> Vector<T>::Vector(const Vector &&other) : Vector(other) {
  ;
}

template <typename T> Vector<T>::Vector(Vector &&other) {
  if (this == &other) {
    throw out_of_range("DADYA NE NNNADO!");
  }
  size_ = other.size_;
  size_max_ = other.size_max_;
  data_ = other.data_;
...
typename Vector<T>::Vector &Vector<T>::operator=(Vector &&other) {
  if (this == &other) {
    throw out_of_range("Doesn't call itself");
  }
  size_ = other.size_;
  size_max_ = other.size_max_;
  if (data_ != nullptr) {
...
template <typename T>
typename Vector<T>::Vector &Vector<T>::operator=(const Vector &other) noexcept {
  if (this != &other) {
    size_ = 0;
    if (data_ != nullptr) {
      delete[] data_;
...
template <typename T> void Vector<T>::AddMemmory_(size_type size) {
  size_type t = -1;
  if (size != t) {
    size_max_ = size;
  } else if (!size_max_) {
...
template <typename T> typename Vector<T>::iterator Vector<T>::data() const {
  return data_;
}

template <typename T> bool Vector<T>::empty() const { return !size_; }
...
template <typename T> void Vector<T>::PointPrint() const {
  for (int i = 0; begin() + i != end(); ++i) {
    cout << "[" << i << "] = " << begin() + i << endl;
  }
}
