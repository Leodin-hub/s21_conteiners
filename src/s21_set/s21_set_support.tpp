...
typename Set<Key>::Nodes *Set<Key>::CreateNode_(value_type value,
                                                Nodes *parent) {
...
template <typename Key>
std::pair<typename Set<Key>::Nodes *, bool>
Set<Key>::FindWrite_(const value_type &value, bool passed) {
  std::pair<Nodes *, bool> tmp(FindRead_(value, root_));
  if (!tmp.second && passed) {
    tmp.first = CreateNode_(value, tmp.first);
    tmp.second = true;
...
template <typename Key>
std::pair<typename Set<Key>::Nodes *, bool>
Set<Key>::FindRead_(const value_type &value, Nodes *search) const {
  if (root_ == nullptr) {
...
template <typename Key> void Set<Key>::FixUp_(Nodes *node) {
  if (node != nullptr) {
...
template <typename Key> void Set<Key>::Rotate_(Nodes *node, const bool left) {
...
template <typename Key>
typename Set<Key>::Nodes *Set<Key>::RemoveNodeChildless2_(Nodes *node) {
...
template <typename Key>
typename Set<Key>::Nodes *Set<Key>::RemoveNodeChildless1_(Nodes *node) {
...
template <typename Key> void Set<Key>::RemoveNode_(Nodes *node) {
  ...
  if (this->size_ == 1) {
    ...
    if (childless == 2) {
      result = RemoveNodeChildless2_(node);
    } else if (childless == 1) {
      result = RemoveNodeChildless1_(node);
    }
    if (root_ == node) {
      root_ = result;
    }
...
template <typename Key> bool Set<Key>::FixDown_(Nodes *node) {
  bool result = false;
  Nodes *parent = node->parent;
...
template <typename Key> bool Set<Key>::IsNull_(Nodes *node, int flag) const {
  bool res;
  if (!flag) {
    res = node == nullptr;
  } else if (flag == 1) {
...
template <typename Key> void Set<Key>::Print() const {
  iterator it = begin();
...
template <typename Key> void Set<Key>::RemoveSet_(Nodes *node) {
  if (node == nullptr) {
    return;
  }
