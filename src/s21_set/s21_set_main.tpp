...
template <typename Key> Set<Key>::Set(const Set &&other) : Set(other) { ; }

template <typename Key> Set<Key>::Set(Set &&other) {
  root_ = other.root_;
  begin_ = other.begin_;
  end_ = other.end_;
  size_ = other.size_;
...
template <typename Key> bool Set<Key>::operator==(Set &other) const {
...
template <typename Key> bool Set<Key>::empty() const {
  return root_ == nullptr;
}

template <typename Key> typename Set<Key>::size_type Set<Key>::size() const {
  return size_;
}
...
template <typename Key>
typename std::pair<typename Set<Key>::iterator, bool>
...
template <typename Key> bool Set<Key>::contains(const value_type &value) const {
