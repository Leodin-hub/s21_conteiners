...
template <typename T, std::size_t S> Array<T, S>::Array() : size_(S) {
  if (this->size_ == 0) {
    this->arr_ = nullptr;
  } else {
    this->arr_ = new T[S]();
...
Array<T, S>::Array(std::initializer_list<value_type> const &items) : Array() {
  if (items.size() > S) {
    throw std::out_of_range("Error, array constructor does not accept a sheet "
...
template <typename T, std::size_t S> Array<T, S>::Array(Array &&a) : Array(a) {
  ;
}

template <typename T, std::size_t S>
Array<T, S>::Array(const Array &&a) : Array(a) {
  ;
}

template <typename T, std::size_t S> Array<T, S>::~Array() { Clear_arr_(); }
...
template <typename T, std::size_t S> bool Array<T, S>::empty() const noexcept {
  return this->size_ == 0;
}

template <typename T, std::size_t S> void Array<T, S>::swap(Array &other) {
...
template <typename T, std::size_t S> void Array<T, S>::Print() {
...
template <typename T, std::size_t S> void Array<T, S>::Clear_arr_() {
  delete[] this->arr_;
  this->size_ = 0;
  this->arr_ = nullptr;
}
